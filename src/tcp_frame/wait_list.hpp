#ifndef _WAIT_LIST_HPP
#define _WAIT_LIST_HPP
...
public:
  ...
  T* get()
  {
    ...
    if (_alive)
    {
      ret = this->get_entry(*linked_list_t<T, list_node>::_head.prev);
      this->del(*ret);
      --_num;
    }
    else
  ...
  T* get_from_head()
  {
    ...
    if (_alive)
    {
      ret = this->get_entry(*linked_list_t<T, list_node>::_head.next);
      this->del(*ret);
      --_num;
    }
    else
