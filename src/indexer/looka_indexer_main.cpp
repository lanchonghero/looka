...
int main(int argc, char** argv)
{
  ...
  for (unsigned int i = 0; i < source_names.size(); i++) {
    ...
    lc_source.insert(std::make_pair(source_names[i], source_cfg));
  }
  for (unsigned int i = 0; i < index_names.size(); i++) {
    LookaConfigIndex* index_cfg = new LookaConfigIndex(&parser, index_names[i]);
    lc_index.insert(std::make_pair(index_names[i], index_cfg));
  }
  ...
  for (unsigned int i = 0; i < index_names.size(); i++) {
    ...
    LookaIndexer* indexer = new LookaIndexer(source_cfg, index_cfg);
    indexer->DoIndex();
    delete indexer;
  }
